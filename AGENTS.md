...
2. **Seeding and migrate**: Sync auto-detects an empty CoJSON database (SQL: no rows in any CoJSON data table) and runs genesis seed once; on every startup it runs registry migrate (diff `MAIA_SPARK_REGISTRY` vs live CoValues). Set `PEER_SYNC_STORAGE=pglite` (local) or `postgres` + `PEER_SYNC_DB_URL` (deploy). `SEED_VIBES` defaults to `all` (includes quickjs). **Emergency full reseed:** set `PEER_SYNC_MODE=seed` (clears PGlite DB+blob, or reset Postgres/Tigris manually first) — then unset the var. Local dev: sync watches `.maia` and live-migrates when `NODE_ENV` is not production (Fly sets `NODE_ENV=production`, so watch is off there).
