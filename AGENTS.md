...
2. **First run requires seeding**: Add `PEER_SYNC_MODE=seed` and `PEER_SYNC_STORAGE=pglite` to `.env` for the first run. After successful seed, remove `PEER_SYNC_MODE` or set `PEER_SYNC_MODE=none` so subsequent restarts use persisted data. `SEED_VIBES` defaults to `all` (includes quickjs); override with e.g. `SEED_VIBES=todos,chat,quickjs` to pick specific vibes. **Note:** With PGlite (localhost), `PEER_SYNC_MODE=seed` auto-clears DB and blob before reseeding. With Postgres/Tigris, reset manually. Config changes that require new scaffold data: reset storage and run `PEER_SYNC_MODE=seed` again. One-shot migrate (registry diff to CoValues): `PEER_SYNC_MODE=migrate`. Local dev sync watches `.maia` and live-migrates when `NODE_ENV` is not production (Fly sets `NODE_ENV=production`, so watch is off there).
