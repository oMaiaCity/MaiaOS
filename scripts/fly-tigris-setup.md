...
Empty Postgres + `PEER_SYNC_SEED=true` runs genesis seed; unset afterward. Storage is never cleared automatically on Fly — wipe Postgres/PGlite/Tigris manually for a full reset. All new binary uploads go to Tigris automatically.

## 4. After First Deploy

After a one-shot `PEER_SYNC_SEED=true` deploy, unset that Fly secret so it does not run on every restart.
