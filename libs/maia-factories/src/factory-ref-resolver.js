 ...
 */
export async function normalizeFactoryReferencesWithResolver(
	resolveFn,
	schema,
	visited = new Set(),
) {
	...
	const resolveIfCoId = async (val) => {
		if (typeof val === 'string' && val.startsWith('co_z') && !visited.has(val)) {
			...
			try {
				...
				if (resolved) return await normalizeFactoryReferencesWithResolver(resolveFn, resolved, visited)
			} finally {
				visited.delete(val)
			}
		}
	...
	if (result.properties !== undefined) {
		...
		if (
			...
			for (const [k, v] of Object.entries(result.properties)) {
				next[k] = await normalizeFactoryReferencesWithResolver(resolveFn, v, visited)
			}
	...
	if (result.items !== undefined) {
		...
		if (Array.isArray(result.items)) {
			result.items = await Promise.all(
				result.items.map((item) => normalizeFactoryReferencesWithResolver(resolveFn, item, visited)),
			)
		} else if (result.items && typeof result.items === 'object') {
			result.items = await normalizeFactoryReferencesWithResolver(resolveFn, result.items, visited)
		}
	}
	if (
		...
		for (const [k, v] of Object.entries(result.$defs)) {
			...
			if (defs[k] && typeof defs[k] === 'object' && !Array.isArray(defs[k])) {
				defs[k] = await normalizeFactoryReferencesWithResolver(resolveFn, defs[k], visited)
			}
		}
		result.$defs = defs
	}
