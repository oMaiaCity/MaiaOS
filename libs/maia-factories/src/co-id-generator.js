...
export class CoIdRegistry {
	 ...
	 * @param {string} humanId - Human-readable ID (e.g., "°Maia/factory/actor", "actor_001")
	...
	register(humanId, coId) {
		...
		if (this.registry.has(humanId)) {
			...
			if (existingCoId !== coId) {
				throw new Error(
					`Co-id already registered for ${humanId}: ${existingCoId} (trying to register ${coId})`,
				)
			}
		...
		// Allow one co-id to map to multiple human-readable IDs (e.g., °Maia/factory/actor and https://...)
