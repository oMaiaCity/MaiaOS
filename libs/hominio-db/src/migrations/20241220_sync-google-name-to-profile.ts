...
export async function migrateSyncGoogleNameToProfile(
	...
	try {
		...
		if (currentFirstName || currentLastName) {
			console.log(
				`[Migration] Profile already has name set (${currentFirstName} ${currentLastName}), skipping sync`,
			)
			return
		}
		...
		console.log(
			`[Migration] Synced Google name to profile: firstName="${firstName}", lastName="${lastName}"`,
		)
	} catch (_error) {
