/**
 * Data seeding — each merged bucket is `{ $factory, instances }` from `.data.maia` files.
...
import { ensureFactoriesLoaded, getFactory } from '@MaiaOS/validation/factory-registry.js'
...
function propertySchemaUsesCotext(propSchema) {
	...
	return typeof co === 'string' && co.includes('cotext.factory.maia')
}
...
export async function seedData(account, node, maiaGroup, peer, data, seedRegistry, options = {}) {
	...
	if (!cotextSchemaCoId?.startsWith?.('co_z')) {
		throw new Error('[seedData] °maia/factory/cotext.factory.maia not registered')
	}
	...
	for (const [collectionName, payload] of Object.entries(data)) {
		...
		if (collectionName === 'icons') {
			...
			if (!Array.isArray(instances)) {
				throw new Error('[seedData] icons: expected instances[] from icons.data.maia')
			}
			let itemCount = 0
			const coIds = []
			for (const item of instances) {
				...
				const bracketRef = `°maia/data/icons.data.maia[${n}]`
				if (instanceCoIdMap) {
