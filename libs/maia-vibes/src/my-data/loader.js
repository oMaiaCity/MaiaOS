/**
 * My Data Vibe Loader
 * Boots MaiaOS with CoJSON backend, loads vibe from account.vibes.myData
...
import { MyDataVibeRegistry } from './registry.js';

/**
 * Load and boot the My Data vibe
 * @param {HTMLElement} container - Container element to render into
 * @returns {Promise<{os: MaiaOS, vibe: Object, actor: Object}>}
 */
export async function loadMyDataVibe(container) {
  ...
  if (existingSession) {
    ...
    os = await MaiaOS.boot({
      node,
      account,
      modules: ['db', 'core'], // db module provides @db tool
      registry: MyDataVibeRegistry  // Registry passed but seeding skipped for CoJSON backend
    });
  }
  
  // Load My Data Vibe from account.vibes.myData using abstracted operations API
  const { vibe, actor: myDataActor } = await os.loadVibeFromAccount(
    'myData', // Vibe key in account.vibes
    container
  );
...
export { MaiaOS, MyDataVibeRegistry };
