/**
 * Chat Vibe Loader
 * Boots MaiaOS with CoJSON backend, loads vibe from account.vibes.chat
...
/**
 * Load and boot the Chat vibe
 * @param {HTMLElement} container - Container element to render into
 * @returns {Promise<{os: MaiaOS, vibe: Object, actor: Object}>}
 */
export async function loadChatVibe(container) {
  console.log('🚀 Booting MaiaOS for Chat Vibe...');
  ...
  if (existingSession) {
    ...
    os = await MaiaOS.boot({
      ...
      registry: ChatVibeRegistry  // Registry passed but seeding skipped for CoJSON backend
    });
  }
  
  // Load Chat Vibe from account.vibes.chat using abstracted operations API
  const { vibe, actor: chatActor } = await os.loadVibeFromAccount(
    'chat', // Vibe key in account.vibes
    container
  );
  
  
  return { os, vibe, actor: chatActor };
}

// Export MaiaOS and registry for convenience
export { MaiaOS, ChatVibeRegistry };
