/**
 * DB Vibe Loader
 * Boots MaiaOS with CoJSON backend, loads vibe from account.vibes.db
...
/**
 * Load and boot the DB vibe
 * @param {HTMLElement} container - Container element to render into
 * @returns {Promise<{os: MaiaOS, vibe: Object, actor: Object}>}
 */
export async function loadDbVibe(container) {
  ...
  if (existingSession) {
    ...
    os = await MaiaOS.boot({
      ...
      registry: DbVibeRegistry  // Registry passed but seeding skipped for CoJSON backend
    });
  }
  
  // Load DB Vibe from account.vibes.db using abstracted operations API
  const { vibe, actor: dbActor } = await os.loadVibeFromAccount(
    'db', // Vibe key in account.vibes
    container
  );
