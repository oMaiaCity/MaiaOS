 ...
 *   maia.db({op: 'update', schema: 'co_z...', id: 'co_z...', data: {done: true}})  // Schema provided explicitly
 * Note: Schema parameter is optional - if not provided, extracted from CoValue headerMeta (single source of truth)
 * 
...
export class UpdateOperation {
   ...
   * @param {Object} params
   * @param {string} [params.schema] - Schema co-id (co_z...) - Optional, extracted from CoValue headerMeta if not provided
  ...
  async execute(params) {
    ...
    // Extract schema co-id - if not provided, extract from CoValue headerMeta (single source of truth)
    // Schema parameter is optional - if provided, use it; otherwise extract from CoValue headerMeta
    let schemaCoId = schema;
    if (!schemaCoId) {
      // Extract schema co-id from CoValue headerMeta (stored in rawExistingData.$schema)
      schemaCoId = rawExistingData.$schema;
      if (!schemaCoId || !schemaCoId.startsWith('co_z')) {
        throw new Error(`[UpdateOperation] Failed to extract schema co-id from CoValue ${id} headerMeta. CoValue must have $schema in headerMeta.`);
      }
    } else {
      // Validate provided schema is a co-id (runtime code must use co-ids only)
      if (!schemaCoId.startsWith('co_z')) {
        throw new Error(`[UpdateOperation] Schema must be a co-id (co_z...), got: ${schemaCoId}. Runtime code must use co-ids only, not '@schema/...' patterns.`);
      }
    }
    
    // Load schema definition for validation (use fromCoValue to ensure we get the correct schema)
