...
/**
 * Bun-native build for aven-client.mjs + aven-server.mjs + pglite.wasm.
...
const buildLog = createLogger('kernel-distros')
...
async function build(entry, outfile, target, opts = {}) {
	...
	if (!entryOutput) {
		buildLog.error('No entry output')
		process.exit(1)
	}
	...
	buildLog.log(`Built ${outfile} (${target})`)
}

async function main() {
	await build('libs/kernel/client/index.js', 'aven-client.mjs', 'browser', {
		plugins: [browserMaiaStorageExportsPlugin()],
	})
	// Server runtime is Bun only — sync bundle must use target `bun` (Bun builtins, SQL, etc.).
	await build('services/sync/src/index.js', 'aven-server.mjs', 'bun')

	const wasmSource = resolvePgliteWasmPath(repoRoot)
	const wasmDest = join(outputDir, 'pglite.wasm')
	if (!existsSync(wasmSource)) {
		buildLog.error('[kernel-distros] Missing pglite.wasm at', wasmSource)
		process.exit(1)
	}
	cpSync(wasmSource, wasmDest)
	buildLog.log('Vendored pglite.wasm')

	buildLog.log('Aven OS build complete')
}
