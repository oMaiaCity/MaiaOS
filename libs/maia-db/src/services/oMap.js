...
export async function createCoMap(group, init = {}, schemaName) {
	...
	// Special case: Meta schema uses hardcoded "@meta-schema" reference (no validation needed)
	if (schemaName === '@meta-schema') {
		const meta = { $schema: '@meta-schema' };
		const comap = group.createMap(init, meta);
		console.log("✅ CoMap created (meta schema):", comap.id);
