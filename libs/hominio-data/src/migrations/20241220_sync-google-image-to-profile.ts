		...
		for (let attempt = 1; attempt <= maxRetries; attempt++) {
			try {
				...
				if (!response.ok) {
					...
					throw new Error(
						`Failed to fetch image: ${response.status} ${errorData.error || response.statusText}`,
					)
				}
				...
				console.log(
					`[Migration] Image downloaded successfully (${imageBlob.size} bytes, ${imageData.mimeType})`,
				)
