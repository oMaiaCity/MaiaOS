...
import { resolveCoValue, type ResolvedCoValueResult } from "./resolve-covalue.js";
...
export async function navigateToCoValueContext(
  ...
  for (const [key, value] of Object.entries(resolved.snapshot)) {
    ...
    // Check if value is an array of CoIDs
      ...
      const allCoIDs = value.every(
        (item) => typeof item === "string" && item.startsWith("co_"),
      );
      if (allCoIDs) {
  ...
  const childrenWithResolved = await Promise.all(
    directChildren.map(async (child) => {
      try {
        ...
        console.warn(`Failed to resolve child ${child.coValueId}:`, e);
        return child;
      }
    }),
  );
