<script lang="ts">
  ...
  const ownerGroup = $derived(
    groupInfo?.groupId ? new CoState(Group, groupInfo.groupId) : null
  );
...
{#if coValue?.$isLoaded}
  <div
    class="relative overflow-hidden rounded-3xl backdrop-blur-xl bg-slate-100/90 border border-white shadow-[0_0_8px_rgba(0,0,0,0.03)] p-5 transition-transform hover:scale-[1.01] duration-300 min-w-[500px]"
  >
    ...
    <div class="relative space-y-5">
      ...
      <div class="flex justify-between items-start gap-4">
        <div class="min-w-0 flex-1">
          ...
          <div class="text-[10px] font-mono text-slate-400 mt-1 truncate opacity-70 hover:opacity-100 transition-opacity">
            ID: {coValue.$jazz.id}
          </div>
        </div>
      </div>
      ...
      <!-- Properties Section -->
      <div class="bg-slate-50/40 rounded-2xl p-4 border border-white shadow-[0_0_4px_rgba(0,0,0,0.02)] backdrop-blur-sm">
        <div class="space-y-3 text-sm">
          <div class="flex justify-between items-center">
            <span class="text-xs font-medium text-slate-500 uppercase tracking-wide">Schema</span>
            {#if coValue.$jazz.has("@schema")}
              <span class="font-medium text-slate-700 bg-slate-50/60 px-2 py-0.5 rounded-md border border-white">{coValue["@schema"]}</span>
            {:else}
            ...
            <div class="flex justify-between items-center pt-2 border-t border-white/50">
              <label for={`name-${coValue.$jazz.id}`} class="text-xs font-medium text-slate-500 uppercase tracking-wide mt-1">Name</label>
              <input
                type="text"
                id={`name-${coValue.$jazz.id}`}
                class="bg-transparent border-b border-white/50 focus:border-slate-300/60 hover:bg-white/30 focus:bg-white/50 rounded px-2 py-1 text-sm font-semibold text-right text-slate-700 outline-hidden transition-all w-1/2 min-w-[150px]"
                value={coValue.name}
                oninput={(e) => {
      ...
      {#if groupInfo && groupInfo.groupId}
        <div class="bg-slate-50/40 rounded-2xl p-4 border border-white shadow-[0_0_4px_rgba(0,0,0,0.02)]">
          <div class="flex items-center justify-between mb-3">
            <div class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Access Control</div>
          ...
          <div class="space-y-3">
            <div class="flex justify-between items-center text-[10px] text-slate-400">
               <span>Group ID</span>
               <span class="font-mono opacity-60">{groupInfo.groupId.slice(0, 8)}...{groupInfo.groupId.slice(-8)}</span>
            </div>
            ...
            {#if groupInfo.accountMembers.length > 0}
              <div class="space-y-1.5">
                {#each groupInfo.accountMembers as member}
                  <div class="flex items-center justify-between p-1.5 rounded-lg hover:bg-white/40 transition-colors group">
                    <div class="flex items-center gap-2 min-w-0">
                      <div class="w-1.5 h-1.5 rounded-full bg-slate-500 shadow-[0_0_4px_rgba(51,65,85,0.3)]"></div>
                      <span class="font-mono text-[10px] text-slate-600 truncate opacity-80 group-hover:opacity-100">
                        {member.id}
                      </span>
                    </div>
            ...
            {#if groupInfo.groupMembers.length > 0}
              <div class="space-y-1.5 pt-2 border-t border-white/50">
                {#each groupInfo.groupMembers as member}
                  <div class="flex items-center justify-between p-1.5 rounded-lg hover:bg-white/40 transition-colors group">
                    <div class="flex items-center gap-2 min-w-0">
                      <div class="w-1.5 h-1.5 rounded-full bg-green-400 shadow-[0_0_4px_rgba(74,222,128,0.3)]"></div>
                      <span class="font-mono text-[10px] text-slate-600 truncate opacity-80 group-hover:opacity-100">
                        {member.id}
                      </span>
                    </div>
