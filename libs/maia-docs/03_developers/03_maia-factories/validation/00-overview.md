...
2. Initialize AJV
   ...
   └─> Registers CoJSON meta-schema (@factory/meta)
...
4. Registers CoJSON meta-schema (@factory/meta)
...
```
→ Uses `@factory/meta` validator directly
...
{
  "properties": {
    "actorRef": { "$co": "@factory/actor" }
  }
}
```
→ Resolves `@factory/actor` to get schema
...
- Supports both human-readable IDs (`@factory/...`) and co-ids (`co_z...`)
...
const resolver = async (id) => {
  // id could be '@factory/actor' or 'co_z123...'
  // Operations API uses universal resolver internally
  const factoryStore = await dbEngine.execute({ op: 'factory', coId: id });
...
const schema = await backend.resolveSchema('@factory/actor');
// or
...
**How it's used:**
1. During `loadSchema()`, if schema has `$factory: "co_z123..."`, resolver loads the meta-schema
