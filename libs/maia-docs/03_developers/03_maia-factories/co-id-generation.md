...
- Generated by CoJSON runtime (not by maia-factories)
- CoIdRegistry stores mappings for lookup during seeding
...
**In MaiaOS:**
- **Human-readable:** `°Maia/factory/actor` (easy to read, but not unique globally)
- **Co-id:** `co_z9h5nwiNynbxnC3nTwPMPkrVaMQ` (unique globally, but harder to read)

**During seeding:**
- Co-ids are produced by the CoJSON runtime when creating CoValues
- CoIdRegistry stores mappings (human-readable → co-id) for lookup
- transformForSeeding replaces human-readable refs with co-ids using the registry
...
- Usage in seeding: `libs/maia-runtime/src/o/engines/db-engine/backend/indexeddb.js`
