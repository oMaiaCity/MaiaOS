...
**In MaiaOS:**
- **Human-readable:** `@factory/actor` (easy to read, but not unique globally)
...
```javascript
import { CoIdRegistry, generateCoId } from '@MaiaOS/factories/co-id-generator';
...
for (const [name, schema] of Object.entries(schemas)) {
  ...
  const schemaKey = `@factory/${name}`;
  
  factoryCoIdMap.set(schemaKey, coId);
...
```javascript
try {
  registry.register('@factory/actor', coId1);
  registry.register('@factory/actor', coId2);  // Throws error!
} catch (error) {
  // Error: Co-id already registered for @factory/actor: co_z123... (trying to register co_z456...)
}
```
...
try {
  registry.register('@factory/actor', newCoId);
} catch (error) {
  // Use existing co-id instead
  const existingCoId = registry.get('@factory/actor');
}
```
...
// 1. Generate co-ids
const coId = generateCoId(schema);
registry.register('@factory/actor', coId);
