...
Empty Postgres + deploy runs genesis seed automatically. All new binary uploads will go to Tigris automatically.

### 4. After First Deploy

Optional: unset `PEER_SYNC_SEED` on Fly after a one-shot seed deploy so it is not applied on every restart.
