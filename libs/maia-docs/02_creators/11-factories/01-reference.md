...
await validateAgainstFactoryOrThrow(schema, instance, context);
```
  ...
  "properties": {
    "attrs": {
      ...
      "additionalProperties": {
        "anyOf": [
          {
            "$co": "@factory/maia-script-expression"  // ← Expression in attributes
          },
          {
            "type": "object",
            "additionalProperties": {
              "$co": "@factory/maia-script-expression"  // ← Nested expression
            }
          }
        ]
      }
...
- Example: `guard.factory.json` uses `additionalProperties: true` to accept any MaiaScript expression properties

**`oneOf` and Schema Alternatives:**
- `oneOf` allows a value to match one of several schema alternatives
- Used in `action.factory.json` to support tool invocations, context updates, or data mapping
  ...
  "properties": {
    ...
    "todos": {
      ...
      "properties": {
        "schema": {
          "type": "string",
          "description": "Schema reference (e.g., '@factory/todos')"
        },
        "filter": {
          "oneOf": [
            {"type": "object"},
        ...
        "properties": {
          "entry": {
            "oneOf": [
              { "type": "object" },
              { "type": "array" },
              { "$co": "@factory/action" },  // ← CoValue reference
              {
                "type": "array",
                "items": { "$co": "@factory/action" }  // ← Array of CoValue references
              }
            ]
          },
          "on": {
              ...
              "oneOf": [
                { "type": "string" },
                {
                  ...
                  "properties": {
                    "target": { "type": "string" },
                    "guard": {
                      "$co": "@factory/maia-script-expression"  // ← Expression reference
                    },
                    "actions": {
                      "type": "array",
                      "items": {
                ...
                { "$co": "@factory/transition" }  // ← CoValue reference
              ]
            }
          }
        }
...
The MaiaOS factories system provides:
