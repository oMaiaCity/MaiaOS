<script>
    ...
    // Props
    export let variant = 'primary'; // primary, secondary, accent, success, warning, alert, info, slate, glass
    export let size = 'md'; // sm, md, lg, xl
    export let rounded = 'full'; // full, lg, md, sm
    export let icon = undefined; // Iconify icon string
    ...
    // State mappings (from parent/FSM)
    export let state = 'inactive'; // active, inactive (for toggle buttons)
    ...
    // Computed Classes
    $: isIconOnly = icon && !$$slots.default;
    $: sizeClass = isIconOnly ? sizeStyles.iconOnly[size] : sizeStyles[size];
    $: widthClass = fullWidth ? css({ w: 'full' }) : '';
    $: stateClass = state === 'active' ? css({ ring: '2px', ringColor: 'accent.500', ringOffset: '2px' }) : ''; // Example active state style
