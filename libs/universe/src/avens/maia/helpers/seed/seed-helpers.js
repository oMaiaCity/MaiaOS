...
import { metaFactorySchemaRaw } from '../meta-factory-schema-raw.js'
...
function mergedMetaSchemaForSeeding() {
	if (!mergedMetaSchemaCache) {
		mergedMetaSchemaCache = withCanonicalFactorySchema(metaFactorySchemaRaw, 'meta.factory.json')
	}
	return mergedMetaSchemaCache
}

/**
 * Find all `°maia/factory/*.factory.json` string refs in a schema (any depth; strings + $co).
...
function findCoReferences(obj, visited = new Set()) {
	if (typeof obj === 'string' && obj.startsWith('°maia/factory/') && obj.includes('.factory.json')) {
		return new Set([obj])
	}
...
function resolveFactoryRefToMapKey(d, selfKey, uniqueSchemasByLabel) {
	if (typeof d !== 'string' || !d.startsWith('°maia/factory/') || !d.includes('.factory.json')) {
		return null
	}
...
/**
 * Topologically sort schema keys by `°maia/factory/*.factory.json` cross-refs in each schema
...
export function sortFactoriesByDependency(
	...
	excludeKeys = ['°maia/factory/meta.factory.json'],
) {
	/** @type {Set<string>} */
