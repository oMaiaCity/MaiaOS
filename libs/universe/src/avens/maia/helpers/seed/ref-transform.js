...
/** Actor instance targets in @actors: file path ending with actor.json (e.g. …/intent.actor.json or …/actor.json) */
const ACTOR_REF_FILE = /^[@°].*actor\.json$/
...
/** °maia/factory/*.factory.json → same nanoid as withCanonicalFactorySchema(basename). */
		...
		for (const [namekey, actorRef] of Object.entries(obj['@actors'])) {
			if (typeof actorRef === 'string' && !actorRef.startsWith('co_z')) {
				if (!isActorTargetRef(actorRef))
					throw new Error(
						`[SchemaTransformer] context.@actors[${namekey}] must be a file path ending in .actor.json, got: ${actorRef}`,
					)
