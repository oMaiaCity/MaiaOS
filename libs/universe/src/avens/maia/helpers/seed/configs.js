...
export async function seedConfigs(
	...
	const createConfig = async (config, configType, path) => {
		...
		// Keep `cotype` in persisted comap data when the schema JSON defines it (e.g. interface). Do not strip —
	...
	const seedWasmConfigs = async () => {
		...
		const cotextSchemaCoId = factoryCoIdMap.get(maiaIdentity('cotext.factory.json').$nanoid)
		const wasmSchemaCoId = factoryCoIdMap.get(maiaIdentity('wasm.factory.json').$nanoid)
