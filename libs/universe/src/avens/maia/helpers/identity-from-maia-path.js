...
function assertSparkRelPath(sparkRelPath) {
	...
	if (t.startsWith('°')) {
		throw new Error(
			'[maiaIdentity] sparkRelPath must not start with ° (pass path under spark maia/, e.g. services/ai/actor.json)',
		)
	}
...
function executableKeyFromNormalizedPath(norm) {
	const intentRoot = /^maia\/([^/]+)\/intent\/intent\.actor\.json$/.exec(norm)
	if (intentRoot) return `${intentRoot[1]}/intent`
	const dottedActor = /^maia\/(.+)\/([^/.]+)\.actor\.json$/.exec(norm)
	if (dottedActor) return `maia/${dottedActor[1]}/${dottedActor[2]}`
	const actorMaia = /^maia\/(.+)\/actor\.json$/.exec(norm)
...
/**
 * @param {string} sparkRelPath - Path under spark maia root, e.g. `services/ai/actor.json`, `actor.factory.json`
 ...
 * @param {string} sparkRelPath - Instance path relative to spark maia root (e.g. services/sandboxed-add/wasm.json)
 */
...
export function logicalRefToSeedNanoid(ref) {
	...
	if (pathUnderMaia.startsWith('factory/')) {
		...
		if (rest.endsWith('.factory.json') && !rest.includes('/')) {
...
/** Registry nanoid for a factory logical ref `°maia/factory/*.factory.json`. */
export function maiaFactoryRefToNanoid(factoryLogicalRef) {
	...
	if (!basename.endsWith('.factory.json')) return null
...
/**
 * @param {string} basename - e.g. actor.factory.json
...
/**
 * @param {object} raw - Parsed schema JSON object
 ...
 * @param {string} relativePath - Basename e.g. actor.factory.json
 */
