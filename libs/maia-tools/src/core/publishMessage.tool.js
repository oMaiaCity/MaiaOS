...
export default {
  async execute(actor, payload) {
    ...
    if (!target) {
      return createErrorResult([createErrorEntry('structural', 'Target is required. Topics infrastructure removed - use direct messaging with target parameter.')]);
    }

    if (!actor.actorEngine) {
      return createErrorResult([createErrorEntry('structural', '[publishMessage] Actor has no actorEngine reference')]);
    }

    if (target.startsWith('@actor/')) {
      return createErrorResult([createErrorEntry('structural', `[publishMessage] Target not transformed: ${target}. Should be a co-id. Check schema transformer.`)]);
    }
