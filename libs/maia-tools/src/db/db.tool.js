...
export default {
  async execute(actor, payload) {
    ...
    if (payload.op === 'create' && payload.schema && !payload.schema.startsWith('co_z')) {
      return createErrorResult([createErrorEntry('structural', `[@db] Schema must be a co-id (co_z...), got: ${payload.schema}`)]);
    }
    ...
    try {
      ...
      return createErrorResult(err.errors ?? [createErrorEntry('structural', err.message || 'Database operation failed')]);
    }
  }
};
