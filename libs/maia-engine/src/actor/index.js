...
export class ActorEngine {
	...
	async updateContextCoValue(actor, updates) {
		...
		if (actor.contextCoId && this.dataEngine) {
			const contextFactoryCoId =
				...
				(await resolveFactoryFromCoValue(this.dataEngine?.peer, actor.contextCoId))
	...
	watchInbox(inboxCoId, actorId, actorConfig) {
		...
		const resolveAndSubscribe = async () => {
			const factoryCoId = await resolveFactoryFromCoValue(this.dataEngine?.peer, inboxCoId)
	...
	async spawnActor(actorConfig, options = {}) {
		...
		if (!configCoId) {
			throw new Error(
				`[ActorEngine] spawnActor: process must be co-id. Got: ${processRef}. Wipe storage and re-run genesis seed if scaffold is wrong.`,
			)
		}
