...
import { resolveFactoryFromCoValue } from '@MaiaOS/db/resolve-helpers'
...
import { requireDataEngine, requireParam, validateCoId, validateItems } from '../ops-assertions.js'
...
async function updateOp(peer, dataEngine, evaluator, params) {
	...
	const factoryCoId = await resolveFactoryFromCoValue(peer, id)
...
async function deleteOp(peer, dataEngine, params) {
	...
	const factoryCoId = await resolveFactoryFromCoValue(dataEngine.peer, id, 'DeleteOperation')
...
async function appendOp(peer, dataEngine, params) {
	...
	const factoryCoId = await resolveFactoryFromCoValue(peer, coId, 'AppendOperation')
	if (!factoryCoId?.startsWith('co_z')) {
		throw new Error(
			`[AppendOperation] CoValue ${coId} has no schema (missing header.meta.$factory). ` +
				'Wipe storage and re-run genesis seed, or ensure the CoValue was created with a valid factory.',
		)
	}
...
async function spliceCoListOp(peer, dataEngine, params) {
	...
	const factoryCoId = await resolveFactoryFromCoValue(peer, coId, 'SpliceCoListOperation')
...
async function uploadBinaryOp(peer, dataEngine, params) {
	...
	const factoryCoId = await resolveFactoryFromCoValue(peer, coId, 'UploadBinaryOperation')
