...
export function glassCard({ hover = true, lifted = false } = {}) {
	let classes = 'overflow-hidden rounded-3xl border border-white/60 bg-white/40 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-xl transition-all duration-500';
	
	if (lifted) {
		classes += ' hover:-translate-y-1 hover:border-white/80 hover:bg-white/50 hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] active:translate-y-0';
	} else if (hover) {
...
export function glassButton({ disabled = false } = {}) {
	let classes = 'rounded-xl border border-white/60 bg-white/40 px-6 py-3 font-medium text-slate-700 shadow-sm backdrop-blur-sm transition-all hover:border-white/80 hover:bg-white/50 hover:shadow-md active:scale-95';
 ...
 * @returns {string} Tailwind classes
 */
export function decorativeBlob(color = 'blue', { position = '', size = 'h-[500px] w-[500px]' } = {}) {
	const colorMap = {
		blue: 'bg-blue-200/20',
