...
function getSparksId(account) {
  ...
  if (!id?.startsWith('co_z')) throw new Error('account.sparks not found. Ensure bootstrap completed.');
  return id;
}
...
async function handleRegisterHuman(worker, body) {
  ...
  if (!username || typeof username !== 'string' || !username.trim()) return err('username required', 400, { validationErrors: [{ field: 'username', message: 'required' }] });
  ...
  try {
    const humansId = await getCoIdByPath(backend, getSparksId(worker.account), ['@maia', 'registries', 'humans'], { retries: 2 });
    const raw = await backend.getRawRecord(humansId);
    if (raw?.[u] != null && raw[u] !== accountId) return err('username already registered to different account', 409);
    const r = await dbEngine.execute({ op: 'update', id: humansId, data: { [u]: accountId } });
    if (r?.ok === false) return err(r.errors?.map((e) => e.message).join('; ') ?? 'update failed', 500);
...
async function handleTrigger(worker, body) {
  ...
  try {
    const r = await dbEngine.execute({ op: 'create', schema: '@maia/schema/data/todos', data: { text, done: false }, spark });
...
async function handleProfile(worker) {
  try {
    ...
    return jsonResponse({ accountId: account?.id ?? account?.$jazz?.id, profileId, sparksId, sparks, profileName: profileName ?? '(not loaded)' });
  } catch (e) {
    return err(e.message, 500);
  }
}
...
async function handleLLMChat(req) {
  ...
  try {
    ...
    if (!messages || !Array.isArray(messages) || messages.length === 0) return jsonResponse({ error: 'messages array required' }, 400);
    ...
    if (!res.ok) {
      ...
      return jsonResponse({ error: data.error || `HTTP ${res.status}`, message: data.message || txt.slice(0, 200) }, 500);
    }
    const data = await res.json();
    const choice = data.choices?.[0];
    return jsonResponse({ content: choice?.message?.content ?? '', role: 'assistant', usage: data.usage ?? null });
  } catch (e) {
    return jsonResponse({ error: 'Failed to process LLM request', message: e?.message ?? 'Unknown' }, 500);
  }
}
...
function startPing(ws) {
  const send = () => {
    if (ws.readyState === WebSocket.OPEN) {
      try { ws.send(JSON.stringify({ type: 'ping', time: Date.now(), dc: 'unknown' })); } catch (e) { clearInterval(iv); }
...
Bun.serve({
  ...
  async fetch(req, srv) {
    ...
    } else if (url.pathname.startsWith('/on-added') || url.pathname.startsWith('/register-human') || url.pathname.startsWith('/trigger') || url.pathname.startsWith('/profile')) {
    ...
    syncHandler = {
      async open(ws) {
        ...
        const peer = createWebSocketPeer({
          ...
          onSuccess: () => {},
          onClose: () => { if (ws.data?.pingInterval) clearInterval(ws.data.pingInterval); },
        });
        localNode.syncManager.addPeer(peer);
        ws.data = { clientId, peer, pingInterval: startPing(ws) };
        queueMicrotask(() => {
          for (const l of openListeners) try { l({ type: 'open', target: adaptedWs }); } catch (e) { console.error(e); }
        });
      },
      async message(ws, msg) {
        for (const l of ws._messageListeners || []) try { l({ type: 'message', data: msg, target: ws._adaptedWs }); } catch (e) { console.error(e); }
      },
      async close(ws, code, reason) {
        if (ws.data?.pingInterval) clearInterval(ws.data.pingInterval);
        if (ws.data?.peer) try { localNode.syncManager.removePeer(ws.data.peer); } catch (e) {}
      },
    };
