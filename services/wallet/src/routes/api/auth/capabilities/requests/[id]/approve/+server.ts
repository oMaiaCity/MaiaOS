...
/**
 * Approve Capability Request Endpoint
 * 
 * Approves a capability request and redirects to callback URL if provided
 */
export const POST: RequestHandler = async ({ request, params }) => {
    try {
        ...
        // Extract principal
        const approverPrincipal = `user:${session.user.id}` as const;

        // Approve the request
        await api.caps.approveCapabilityRequest(approverPrincipal, requestId);
        ...
        if (callbackUrl) {
            ...
            callbackWithStatus.searchParams.set('status', 'approved');
        ...
        console.error("[capabilities/requests/approve] Error:", error);
