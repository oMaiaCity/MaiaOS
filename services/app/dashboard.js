...
async function readCotextSvgString(maia, cotextCoId) {
	if (typeof cotextCoId !== 'string' || !cotextCoId.startsWith('co_z')) return null
	try {
		const store = await maia.do({ op: 'read', factory: null, key: cotextCoId })
		const v = store?.value ?? store
		const items = v?.items
 ...
 * @returns {Promise<Array>} Array of vibe objects with {key, name, description, coId, iconSvg}
 */
		...
		const vibeEntries = await Promise.all(
			vibeKeys.map(async (vibeKey) => {
				...
				try {
					...
					iconSvg = await resolveVibeIconSvgFromManifest(maia, manifest)
				} catch (_e) {}
				return { key: vibeKey, name, description, coId: vibeCoId, iconSvg }
			}),
		)
	...
	try {
		...
		if (currentVibe?.startsWith?.('co_z')) {
			try {
				await perf.measure('vibeManifestTitle', async () => {
					...
					vibeNavbarIconSvg = await resolveVibeIconSvgFromManifest(maia, manifest)
				})
			} catch (_e) {
		...
		<div class="db-container">
			...
			<header class="db-header whitish-card">
				<div class="header-content">
					<div class="header-left">
						<span class="db-header-maia-icon" aria-hidden="true">${vibeNavbarIconSvg}</span>
