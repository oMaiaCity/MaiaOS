<script lang="ts">
  ...
  if (browser) {
    try {
      ...
      console.error("Failed to get Jazz context during initialization:", error);
    }
  }
  ...
  onMount(async () => {
    if (!browser) return;
    if (!authClient.jazz) {
      console.error("Jazz plugin not available on authClient");
      return;
    }
    if (!jazzContext || !authSecretStorage) {
      console.error("Jazz context or auth secret storage not available");
      return;
    }
    ...
    try {
      ...
      console.error("Failed to set up Jazz integration:", error);
    }
  });
</script>
