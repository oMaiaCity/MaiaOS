...
const recipientButtonLeaf: LeafNode = {
	tag: 'div',
	classes: ['relative'],
	children: [recipientButtonSelectedLeaf, recipientButtonUnselectedLeaf],
}
		...
		// Form content
		{
			...
			children: [
				...
				// Send button - Always visible, disabled when conditions not met
				{
					...
					bindings: {
						disabled:
							"!selectedRecipient || !sendAmount || sendAmount === '' || sendAmount === 0 || (typeof sendAmount === 'number' && sendAmount <= 0) || (typeof sendAmount === 'string' && (sendAmount.trim() === '' || Number(sendAmount) <= 0 || isNaN(Number(sendAmount))))",
					},
					classes: [
						'w-full',
						'px-4',
