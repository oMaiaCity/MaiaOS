	...
	children: [
		{
			...
			children: [
				...
				{
					tag: 'div',
					classes: ['flex', 'items-center', 'justify-between', 'p-6', 'border-b', 'border-slate-200'],
					children: [
						{
							tag: 'h2',
				...
				// Modal content
				{
					...
					children: [
						...
						// Send button - Always visible, disabled when conditions not met
						{
							...
							bindings: {
								disabled:
									'data.selectedRecipient === null || !data.sendAmount || String(data.sendAmount).trim().length === 0',
							},
							classes: [
								'w-full',
								'px-4',
